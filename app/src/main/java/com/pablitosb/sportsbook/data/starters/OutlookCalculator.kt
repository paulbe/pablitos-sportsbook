 ...
 * - expectedBF = season BF/GS, else last-start BF, else 26; clamp 15–32
...
object OutlookCalculator {
    ...
    fun expectedBf(sample: PitchingSample?): Float {
        return when {
            sample != null && sample.seasonGs > 0 && sample.seasonBf > 0 ->
    ...
    fun project(sample: PitchingSample?): Projection {
        if (sample == null || sample.seasonBf <= 0 && sample.recentBf <= 0) {
            return Projection(
                projK = LEAGUE_K,
                nextStartKs = LEAGUE_K * expectedBf(null),
                outlookScore = 0,
        ...
        return Projection(
            projK = projK,
            nextStartKs = projK * expectedBf(sample),
            outlookScore = score,
            outlook = outlook,
